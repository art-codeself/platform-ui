<template>
  <el-dialog title="职位成员设置" :visible.sync="visible" width="800px" top="20px">
    <el-row :gutter="20" class="position-set">
      <el-col :span="12">
        <p>可添加成员</p>
...
<script>
import { positionMembersUpd, positionMembersAdded, positionMembersNotin } from '@/api/position'
export default {
  name: 'position-user-set',
  data () {
    return {
      positionId: null,
      visible: false,
  ...
  methods: {
    ...
    queryAdded () {
      positionMembersAdded(this.positionId, Object.assign(this.pageAdded, { username: this.deleteUsername }))
        .then(({ data }) => {
    ...
    queryNotin () {
      positionMembersNotin(this.positionId, Object.assign(this.pageNotin, { username: this.addUsername }))
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
    ...
    addOne (row) {
      positionMembersUpd(this.positionId, [row.id], 'add')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
    ...
    deleteOne (row) {
      positionMembersUpd(this.positionId, [row.id], 'delete')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
