...
export default {
  ...
  methods: {
    ...
    queryNotin () {
      roleMembersNotin(this.roleId, Object.assign(this.pageNotin, { username: this.addUsername, deptId: this.deptId }))
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
    ...
    addOne (row) {
      roleMembersUpd(this.roleId, [row.id], 'add')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
    ...
    deleteOne (row) {
      roleMembersUpd(this.roleId, [row.id], 'delete')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
