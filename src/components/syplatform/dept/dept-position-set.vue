<template>
  <el-dialog title="部门职位设置" :visible.sync="visible" width="800px" top="20px">
    <el-row :gutter="20" class="dept-set">
      <el-col :span="12">
        <p>可添加职位</p>
        <div style="margin-bottom: 10px">
          <el-input size="small" @keyup.enter.native="queryNotin" style="width: 200px;margin-right:10px" v-model="addPositionname" clearable></el-input>
      ...
      <el-col :span="12">
        <p>已添加职位</p>
        <div style="margin-bottom: 10px">
          <el-input size="small" @keyup.enter.native="queryAdded" style="width: 200px;margin-right:10px" v-model="deletePositionname" clearable></el-input>
...
<script>
import { deptPositionsUpd, deptPositionsAdded, deptPositionsNotin } from '@/api/dept'
export default {
  name: 'dept-position-set',
  data () {
    return {
      deptId: null,
      visible: false,
  ...
  methods: {
    ...
    queryAdded () {
      deptPositionsAdded(this.deptId, Object.assign(this.pageAdded, { positionname: this.deletePositionname }))
        .then(({ data }) => {
    ...
    queryNotin () {
      deptPositionsNotin(this.deptId, Object.assign(this.pageNotin, { positionname: this.addPositionname }))
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
    ...
    addOne (row) {
      deptPositionsUpd(this.deptId, [row.id], 'add')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
    ...
    deleteOne (row) {
      deptPositionsUpd(this.deptId, [row.id], 'delete')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
