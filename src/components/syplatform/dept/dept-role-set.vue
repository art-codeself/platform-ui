<template>
  <el-dialog title="部门角色设置" :visible.sync="visible" width="800px" top="20px">
...
<script>
import { deptRolesUpd, deptRolesAdded, deptRolesNotin } from '@/api/dept'
export default {
  name: 'dept-role-set',
  data () {
    return {
  ...
  methods: {
    ...
    queryAdded () {
      deptRolesAdded(this.deptId, Object.assign(this.pageAdded, { rolename: this.deleteRolename }))
        .then(({ data }) => {
    ...
    queryNotin () {
      deptRolesNotin(this.deptId, Object.assign(this.pageNotin, { rolename: this.addRolename }))
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
    ...
    addOne (row) {
      deptRolesUpd(this.deptId, [row.id], 'add')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
    ...
    deleteOne (row) {
      deptRolesUpd(this.deptId, [row.id], 'delete')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
