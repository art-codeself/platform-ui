<template>
  <el-dialog title="部门成员设置" :visible.sync="visible" width="800px" top="20px">
...
<script>
import { deptMembersUpd, deptMembersAdded, deptMembersNotin } from '@/api/dept'
export default {
  name: 'dept-user-set',
  data () {
    return {
  ...
  methods: {
    ...
    queryAdded () {
      deptMembersAdded(this.deptId, Object.assign(this.pageAdded, { username: this.deleteUsername }))
        .then(({ data }) => {
    ...
    queryNotin () {
      deptMembersNotin(this.deptId, Object.assign(this.pageNotin, { username: this.addUsername }))
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
    ...
    addOne (row) {
      deptMembersUpd(this.deptId, [row.id], 'add')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
    ...
    deleteOne (row) {
      deptMembersUpd(this.deptId, [row.id], 'delete')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
