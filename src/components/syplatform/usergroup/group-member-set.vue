<template>
  <el-dialog title="用户组成员设置" :visible.sync="visible" width="800px" top="20px">
...
<script>
import { usergroupMembersUpd, usergroupMembersAdded, usergroupMembersNotin } from '@/api/usergroup'
export default {
  ...
  methods: {
    ...
    queryAdded () {
      usergroupMembersAdded(this.groupId, Object.assign(this.pageAdded, { username: this.deleteUsername }))
        .then(({ data }) => {
    ...
    queryNotin () {
      usergroupMembersNotin(this.groupId, Object.assign(this.pageNotin, { username: this.addUsername }))
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
    ...
    addOne (row) {
      usergroupMembersUpd(this.groupId, [row.id], 'add')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
    ...
    deleteOne (row) {
      usergroupMembersUpd(this.groupId, [row.id], 'delete')
        .then(({ data }) => {
          if (data.status === 'SUCCEED') {
            this.queryAdded()
            this.queryNotin()
