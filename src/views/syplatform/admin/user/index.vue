<template>
  <sy-container>
    ...
    <el-table :key='tableKey'
      ...
      <el-table-column align="center" label="所属职位">
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="400px">
      <el-form :model="form" :rules="rules" ref="form" label-width="100px">
        ...
        <el-form-item label="所属部门" prop="deptId">
          <el-input v-model="form.deptName" placeholder="选择部门" :disabled="dialogStatus=='update'" @focus="handleDept()" readonly></el-input>
    ...
    <sy-user-role-set ref="userRoleSet"/>
  </sy-container>
</template>
...
import { deptPositionList } from '@/api/position'
...
export default {
  ...
  methods: {
    ...
    getPositionListByDept (deptId) {
      // 根据部门加载职位
      deptPositionList(deptId).then(response => {
    ...
    getNodeData (data) {
      ...
      this.getPositionListByDept(data.id)
    },
    handleDept () {
    ...
    handleUpdate (row) {
      getObj(row.userId).then(response => {
        ...
        this.getPositionListByDept(this.form.deptId)
      })
    },
    create (formName) {
      const set = this.$refs
    ...
    handleRoleSet (row) {
      this.$refs.userRoleSet.init(row.userId, row.deptId)
    }
  }
}
</script>
