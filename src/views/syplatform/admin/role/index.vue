...
<sy-container>
  ...
  <el-table
    ...
    <el-table-column label="操作" width="320" fixed="right">
      <template slot-scope="scope">
        ...
        <el-button size="mini" type="warning" @click="handleMenu(scope.row)">授权</el-button>
  ...
  <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="400px">
    <el-form :model="form" :rules="rules" ref="form" label-width="100px">
      ...
      <el-form-item label="所属部门" prop="roleDeptId">
        <el-input v-model="form.roleDeptName" :disabled="dialogStatus=='update'" placeholder="选择部门" @focus="handleDept()" readonly></el-input>
        <input type="hidden" v-model="form.roleDeptId" />
      </el-form-item>
  ...
  <sy-role-member-set ref="roleMemberSet"/>
...
export default {
  ...
  methods: {
    ...
    handleMenu (row) {
      fetchRoleTree(row.code)
        .then(response => {
          this.checkedKeys = response.data
          return fetchDeptAuthorizationTree(row.roleDeptId)
        })
        .then(response => {
    ...
    updateRoleMenu (roleId, roleCode) {
      ...
      roleMenuUpd(roleId, currentCheckedKeys).then(() => {
    ...
    handleUserSet (row) {
      this.$refs.roleMemberSet.init(row.id, row.roleDeptId)
    }
  }
}
</script>
