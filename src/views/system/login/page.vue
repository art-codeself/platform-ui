    ...
    <div class="page-login--layer">
      <div
        ...
        <div
          ...
          <h1 class="page-logo-label">{{$t("views.system.login.header.title")}}</h1>
          <el-select v-model="tenant" :placeholder="$t('views.system.login.header.placeholder.tenant')">
            <el-option
          ...
          <el-tabs v-model="activeName">
            <el-tab-pane :label="$t('views.system.login.form.title')" name="first">
              <div class="page-login--form">
                <el-card shadow="never">
                  <el-form ref="loginForm" label-position="top" :rules="rules" :model="formLogin" size="default">
                    <el-form-item prop="username">
                      <el-input type="text" v-model="formLogin.username" :placeholder="$t('views.system.login.form.placeholder.username')">
                        <i slot="prepend" class="fa fa-user-circle-o"></i>
                      </el-input>
                    </el-form-item>
                    <el-form-item prop="password">
                      <el-input type="password" v-model="formLogin.password" :placeholder="$t('views.system.login.form.placeholder.password')">
                        <i slot="prepend" class="fa fa-keyboard-o"></i>
                      </el-input>
                    </el-form-item>
                    <el-form-item prop="code">
                      <el-input type="text" v-model="formLogin.code" :placeholder="$t('views.system.login.form.placeholder.code')">
                        <template slot="prepend">{{$t('views.system.login.form.label.code')}}</template>
                        <template slot="append">
                          <img class="login-code" :src="code.src" @click="refreshCode">
                        </template>
                      </el-input>
                    </el-form-item>
                    <el-button size="default" @click="submit" type="primary" class="button-login">{{$t('views.system.login.form.button.login')}}</el-button>
                  </el-form>
                </el-card>
              </div>
            </el-tab-pane>
            <el-tab-pane label="手机号登录" name="second" v-if="false">
              <div class="page-login--form">
                <el-card shadow="never">
          ...
          <p
            ...
            <span><sy-icon name="question-circle"/> {{ $t('views.system.login.options.forget-password') }}</span>
        ...
        <div class="page-login--content-footer">
          ...
          <p class="page-login--content-footer-copyright">
            Copyright <sy-icon name="copyright"/> 2019 syplatform <a href="#">@</a>
          </p>
        </div>
      </div>
    </div>
...
import { selectList } from '@/api/tenant'
import './style.scss'
export default {
  data () {
    return {
